...
void	parser_export2(char *massiv)
{
	...
	//добавить условие, что сюда parser_env_export идем только в случае если есть = в массиве
	...
	if (sm == 1) // в случае ошибки в ключе
	{
		...
		write(2,"export: ", 8);
		write(2,"\'", 1);
		ft_putstr_fd(massiv, 2); //TODO + очищение памяти
		write(2,"\'", 1);
